...
private:
	int n;																//原始数据长度
	vector<long long> lazy;												//lazy标记
	vector<long long> ans;												//区间和值 或 叶子节点值														
	vector <int> arr;													//输入的原始数据
...
public:
	...
	Segment_tree(int len, vector <int>& nums) {
		n = len;
		lazy.resize((n << 2) + 1);										//初始lazy化数组
		ans.resize((n << 2) + 1);										//初始化ans数组
		arr = move(nums);												//将nums赋值给arr
		bulid(1, 1, n);													//建立线段树
	}
